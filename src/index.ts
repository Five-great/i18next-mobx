...
export class I18NextMobxProxy<T = i18n> {
    ...
    t = (keys: string | string[], options?: TOptions<object>, lastKey?: string) => {
        const keyIdx = Array.isArray(keys) ? keys.join('=') : keys

        this.i18nMap[this.currentLanguage] ||= {}
        this.i18nMap[this.currentLanguage][keyIdx] ||= i18next.t(keys, lastKey, options)

        return this.i18nMap[this.currentLanguage][keyIdx] ?? keys
    }
...
export { isServer, i18next }

export const t = i18nProxy.t
